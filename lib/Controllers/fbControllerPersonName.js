...
if(Meteor.isClient){
  ...
  FormBuilder.controllers.fbControllerPersonName.getValue = function(fieldName, parentID, position){
    var result = [];
    ...
    position.value++;
    return {visible:view.isVisible,  value:result};
  };
  ...
  FormBuilder.controllers.fbControllerPersonName.setError = function(fieldName, parentID, position, message){
    if(typeof message === 'string') message = [message];
    message = message || [];
    var view = FormBuilder.views.findOne({parentID:parentID, fieldName:fieldName, position:position.value});
    var namePosn = {value:0};
    FormBuilder.views.find({parentID:view._id}, {sort: {position : 1 }}).forEach(function(viewDataObj){
      FormBuilder.controllers[viewDataObj.schemaObj.controller].setError(viewDataObj.fieldName, viewDataObj.parentID, namePosn, message[namePosn.value]);
    });
    position.value++;
  };
