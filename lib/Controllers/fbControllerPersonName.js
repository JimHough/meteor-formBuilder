...
if (Meteor.isClient) {
  ...
  obj.prototype.addViews = function (fieldName, formID, schemaObj, position, parentID) {
    ...
    var posn = {value: 0};
    FormBuilder.controllers[schemaObj.controllerText].addViews(fieldName, formID, forenameSchema, posn, baseResult);
    FormBuilder.controllers[schemaObj.controllerArray].addViews(fieldName, formID, middlenamesSchema, posn, baseResult);
    FormBuilder.controllers[schemaObj.controllerText].addViews(fieldName, formID, surnameSchema, posn, baseResult);
    return baseResult;
  };
}
