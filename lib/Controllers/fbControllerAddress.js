...
if(Meteor.isClient){
  ...
  FormBuilder.controllers.fbControllerAddress.getValue = function(fieldName, parentID, position){
    var result = [];
  ...
  FormBuilder.controllers.fbControllerAddress.setError = function(fieldName, parentID, position, message){
    if(typeof message === 'string') message = [message];
    message = message || [];
    var view = FormBuilder.views.findOne({parentID:parentID, fieldName:fieldName, position:position.value});
    var addressPosn = {value:0};
    FormBuilder.views.find({parentID:view._id}, {sort: {position : 1 }}).forEach(function(viewDataObj){
      FormBuilder.controllers[viewDataObj.schemaObj.controller].setError(viewDataObj.fieldName, viewDataObj.parentID, addressPosn, message[addressPosn.value]);
    });
    position.value++;
  };
