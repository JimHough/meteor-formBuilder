...
if(Meteor.isClient){
  ...
  obj.prototype.setValue = function(fieldName, parentID, position, value){
    var viewData = FormBuilder.views.findOne({parentID:parentID, fieldName:fieldName, position:position.value});
    var result = FormBuilder.views.update({_id:viewData._id}, {$set:{currentValue:value}});
    if(viewData.schemaObj.asYouType){
      var error = FormBuilder.controllers[viewData.schemaObj.controller].validate(viewData.fieldName, value, viewData.schemaObj, Mongo.Collection.get(viewData.formObj.collection), viewData.formObj.document);
...
//Gets an object that defines the schema default values
  obj.prototype.getSchemaDefaults = function(){
    return {
      ...
      optional:false,
      filter:0xFFFF,
      valueChanged:function(value){console.log(value);}
    };
  };
